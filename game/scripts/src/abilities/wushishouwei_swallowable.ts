...
@registerAbility()
export class wushishouwei_swallowable extends BaseAbility {
    GetBehavior(): AbilityBehavior | Uint64 {
        return AbilityBehavior.PASSIVE;
    }

    GetIntrinsicModifierName(): string {
        return modifier_wushishouwei_swallowable.name;
    }
}
//吞噬后的技能buff
@registerModifier()
export class modifier_wushishouwei_swallowable extends BaseModifier {
    ...
    OnIntervalThink() {
        ...
        if (this.cd_remaining <= 0 && parent.IsAlive()) {
            //释放技能
            //召唤
            let unit = CreateUnitByName("npc_unit_wushishouwei", parent.GetOrigin(), false, parent, parent.GetOwner(), DotaTeam.GOODGUYS);

            if (unit) {
                unit.AddNewModifier(parent, null, "modifier_wushishouwei", {
                    duration: duration,
                    radius: radius,
...
// 技能效果
@registerModifier()
export class modifier_wushishouwei extends BaseModifier {
    IsHidden(): boolean {
        return false;
    }
    ...
    OnIntervalThink() {
        ...
        if (enemy) {
            ...
            let ParticleID = ParticleManager.CreateParticle("particles/units/heroes/hero_lion/lion_spell_finger_of_death.vpcf",
                ParticleAttachment.ABSORIGIN_FOLLOW, this.GetParent())
            ParticleManager.SetParticleControlEnt(ParticleID, 0, this.GetParent(), ParticleAttachment.POINT_FOLLOW, "attach_hitloc", this.GetParent().GetAbsOrigin(), true)
            ParticleManager.SetParticleControl(ParticleID, 1, enemy.GetAbsOrigin())
            ParticleManager.SetParticleControl(ParticleID, 2, enemy.GetAbsOrigin())
            ParticleManager.ReleaseParticleIndex(ParticleID)
            EmitSoundOnLocationWithCaster(point, "Hero_Lion.FingerOfDeathImpact", this.GetParent())
        }
