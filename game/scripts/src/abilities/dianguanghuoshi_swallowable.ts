...
@registerAbility()
export class dianguanghuoshi_swallowable extends BaseAbility {
    GetBehavior(): AbilityBehavior | Uint64 {
        return AbilityBehavior.PASSIVE;
    }

    GetIntrinsicModifierName(): string {
        return modifier_dianguanghuoshi_swallowable.name;
    }
}
//吞噬后的技能buff
@registerModifier()
export class modifier_dianguanghuoshi_swallowable extends BaseModifier {
    ...
    OnAttack(event: ModifierAttackEvent) {
        if (event.attacker == this.GetParent()) {
            ...
            if (RollPseudoRandomPercentage(this.attack_chance, PseudoRandom.CUSTOM_GENERIC, attacker)) {
                ...
                let effectName = "particles/units/heroes/hero_snapfire/hero_snapfire_shotgun.vpcf";
...
// 技能效果
@registerModifier()
export class modifier_dianguanghuoshi extends BaseModifier {
    IsHidden(): boolean {
        return false;
    }
