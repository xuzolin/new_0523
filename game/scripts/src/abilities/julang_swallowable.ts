...
@registerAbility()
export class julang_swallowable extends BaseAbility {
    GetBehavior(): AbilityBehavior | Uint64 {
        return AbilityBehavior.PASSIVE;
    }

    GetIntrinsicModifierName(): string {
        return modifier_julang_swallowable.name;
    }
}
//吞噬后的技能buff
@registerModifier()
export class modifier_julang_swallowable extends BaseModifier {
    ...
    OnAttack(event: ModifierAttackEvent) {
        if (event.attacker == this.GetParent()) {
            ...
            if (RollPseudoRandomPercentage(this.attack_chance, PseudoRandom.CUSTOM_GENERIC, attacker)) {
                ...
                let effectName = "particles/units/heroes/hero_tidehunter/tidehunter_gush_upgrade.vpcf";
                let projectile_speed = 2500;
                let distance = 1600;
                ...
                attacker.EmitSound("Ability.GushCast")
            }
