...
@registerAbility()
export class danwuxufa_swallowable extends BaseAbility {
    GetBehavior(): AbilityBehavior | Uint64 {
        return AbilityBehavior.PASSIVE;
    }

    GetIntrinsicModifierName(): string {
        return modifier_danwuxufa_swallowable.name;
    }
}
//吞噬后的技能buff
@registerModifier()
export class modifier_danwuxufa_swallowable extends BaseModifier {
    ...
    OnAttack(event: ModifierAttackEvent) {
        if (event.attacker == this.GetParent()) {
            ...
            if (this.count >= this.attack_count) {
                ...
                let effectName = "particles/units/heroes/hero_muerta/muerta_deadshot_linear.vpcf";
    ...
    OnProjectileHit = (target: CDOTA_BaseNPC | undefined, modifier_name?: string): boolean => {
        ...
        if (target) {
            ...
            // target.AddNewModifier(this.GetParent(), this.GetAbility(), "modifier_danwuxufa_debuff", { duration: 1, })
        }
        return false
    }
}
...
@registerModifier()
export class modifier_danwuxufa_debuff extends BaseModifier {
    IsHidden(): boolean {
        return false
    }
