...
@registerAbility()
export class yanmu_swallowable extends BaseAbility {
    GetBehavior(): AbilityBehavior | Uint64 {
        return AbilityBehavior.PASSIVE;
    }

    GetIntrinsicModifierName(): string {
        return modifier_yanmu_swallowable.name;
    }
}
//吞噬后的技能buff
@registerModifier()
export class modifier_yanmu_swallowable extends BaseModifier {
    ...
    OnIntervalThink() {
        ...
        if (this.cd_remaining <= 0 && parent.IsAlive()) {
            ...
            if (targets.length > 0) {
                ...
                let thinker = CreateModifierThinker(parent, null, "modifier_yanmu", {
                    duration: duration,
                    radius: radius,
                ...
                // let particle = ParticleManager. CreateParticle(
                //     "particles/units/heroes/hero_riki/riki_smokebomb.vpcf", 
                //     ParticleAttachment.WORLDORIGIN, thinker)
                // ParticleManager.SetParticleControl(particle, 0, thinker.GetAbsOrigin())
                // ParticleManager.SetParticleControl(particle, 1, Vector(aoe_radius, 0, aoe_radius))
...
// 技能效果
@registerModifier()
export class modifier_yanmu extends BaseModifier {
    IsHidden(): boolean {
        return false;
    }
    ...
    OnCreated(params: any): void {
        ...
        const particleId = ParticleManager.CreateParticle(
            // "particles/units/heroes/hero_riki/riki_smokebomb_smoke.vpcf",
            "particles/econ/items/riki/riki_head_ti8/riki_smokebomb_ti8.vpcf",
    ...
    // 光环
    IsAura() { return true; }
    GetModifierAura() { return "modifier_yanmu_aura_debuff"; }
    GetAuraRadius() {
...
@registerModifier()
export class modifier_yanmu_aura_debuff extends BaseModifier {
    IsHidden(): boolean {
        return true;
    }
    IsDebuff(): boolean {
    ...
    DeclareFunctions() {
        return [
            ModifierFunction.MISS_PERCENTAGE,   // 闪避率
        ]
    }

    GetModifierMiss_Percentage() {   // 闪避率
        return 30
    }
}
